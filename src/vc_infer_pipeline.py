...
class VC(object):
    ...
    def vc(
        ...
        # HuBERT must run on CPU — fairseq uses ops unsupported on MPS.
        # Send features to CPU for extraction, then move back to target device.
        hubert_device = 'cpu'
        inputs = {
            "source": feats.to(hubert_device).float(),
            "padding_mask": padding_mask.to(hubert_device),
        ...
        # Move features back to target device
